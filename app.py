...
@app.route("/check")
def check():
    # Create a list of numbers from 1 to 11300 in ascending order
    numbers = list(range(1, 11201))

    sample_ids = db.session.query(sample_stock.sample_id).all()

    # Flatten the list of tuples returned by the query
    sample_ids = [item[0] for item in sample_ids]

    # Find the values that are in the list but not in sample_id
    missing_values = [num for num in numbers if num not in sample_ids]

    # Return the missing values as a JSON response
    return jsonify(missing_values)
